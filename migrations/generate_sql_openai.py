...
def generate_statement(template: str, fecha: str, monto: float, alumno_id: int) -> str:
    """Produce a customized INSERT statement for one student.

    Starting from the given template, this function substitutes the
    ``Fecha``, ``Monto`` and ``AlumnoId`` values.  Only the first
    occurrence of ``CURRENT_TIMESTAMP`` (for the ``Fecha`` field), the
    first monetary literal (``100.00``) and the first numeric literal
    following a comma (``7,``) are replaced.  The rest of the
    ...
    monto: float
        Amount to substitute for the ``Monto`` field.  Two decimal
        places are emitted.
    alumno_id: int
    ...
    -------
    str
        A fully formatted INSERT statement for the given student.
    """
    out = template
    # Replace only the first occurrence of CURRENT_TIMESTAMP with our date
    out = out.replace('CURRENT_TIMESTAMP', f"'{fecha}'", 1)
    # Replace the first monetary literal (assumes default 100.00 in template) with our amount
    # Use a regular expression to find a number with optional decimals followed by a comma
    out = re.sub(r'\b\d+\.\d+\b', lambda m: f"{monto:.2f}" if m.group(0) else m.group(0), out, count=1)
    # Replace the first occurrence of a number (AlumnoId) after a newline and some spaces
    # We locate the line containing AlumnoId by its comment
    out = re.sub(r'\n(\s*)(\d+),\s*-- AlumnoId',
                lambda m: f"\n{m.group(1)}{alumno_id},                              -- AlumnoId", out, count=1)
    return out
...
def main() -> None:
    ...
    for row in range(3, 22):
        ...
        stmt = generate_statement(template, fecha, monto, alumno_id)
