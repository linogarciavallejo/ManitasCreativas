...
try
{
    ...
    Log.Information(
        "AWS Configuration - AccessKey: {AccessKey}, Region: {Region}, SecretKey Length: {SecretKeyLength}",
    ...
    builder.Services.AddCors(options =>
    {
        options.AddPolicy(
            ...
            {
                builder
                    .WithOrigins("http://localhost:5173", "http://localhost:5174") // Allow specific origins
                    .AllowAnyMethod()
                    .AllowAnyHeader()
    ...
    builder.Services.AddScoped<
        IEntradaUniformeDetalleRepository,
        EntradaUniformeDetalleRepository
    >();
    ...
    builder.Services.AddScoped<IPagoService, PagoService>(sp =>
    {
        ...
        return new PagoService(
            ...
            rubroRepository,
            usuarioRepository,
            pagoImagenRepository,
            alumnoContactoRepository,
            alumnoRutaRepository,
    ...
    try
    {
        ...
        Log.Warning(
            "This is a test WARNING log for CloudWatch from {Environment}",
            app.Environment.EnvironmentName
        );
        Log.Error(
            "This is a test ERROR log for CloudWatch from {Environment}",
            app.Environment.EnvironmentName
        );
        Log.Information(
            "Application started in {Environment} mode at {StartTime}",
            app.Environment.EnvironmentName,
            DateTime.UtcNow
        );
    ...
    Log.Information(
        "ManitasCreativas Web API started successfully in {Environment} mode",
        app.Environment.EnvironmentName
    );
