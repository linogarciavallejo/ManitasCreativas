...
const Statement: React.FC = () => {
  ...
  const [selectedAlumno, setSelectedAlumno] = useState<AlumnoSimpleDto | null>(
    null
  );
  ...
  useEffect(() => {
    const fetchAlumnos = async () => {
      try {
        const data = await makeApiRequest<AlumnoSimpleDto[]>(
          "/alumnos/full",
          "GET"
        );
        setAlumnos(data);
  ...
  useEffect(() => {
    const fetchAlumnoData = async () => {
      ...
      try {
        ...
        const details = await makeApiRequest<AlumnoDto>(
          `/alumnos/codigo/${selectedAlumno.codigo}`,
          "GET"
        );
  ...
  const handleCodigoSearch = async (codigo: string) => {
    ...
    try {
      ...
      const alumno = await makeApiRequest<AlumnoDto>(
      ...
      try {
        ...
        console.log("Error fetching statement, student might have no payments:", statementError);
        // Student exists but has no payments - this is not an error, just show empty statement
        setStatements([]);
        toast.warning("El estudiante no tiene pagos registrados hasta el momento.");
      }
    } catch (error) {
  ...
  const onSelect = (value: string) => {
    ...
    const selected = alumnos.find(
      (alumno) =>
        alumno.fullName === selectedText ||
        `${alumno.fullName} (${alumno.codigo})` === selectedText
    );
  ...
  const getPaymentStatus = (
    payment: PagoReadDto
  ): { status: "ontime" | "late" | "verylate"; daysPastDue: number | null } => {
    ...
    },
    {
      ...
      render: (_: string, record: PagoReadDto) => (
        ...
        >
          ...
          {record.esColegiatura &&
            record.mesColegiatura &&
            record.anioColegiatura &&
    ...
    },
    {
      ...
      render: (_: unknown, record: PagoReadDto) =>
        record.imagenesPago && record.imagenesPago.length > 0 ? (
          ...
          >
            Ver {record.imagenesPago.length}{" "}
            {record.imagenesPago.length === 1 ? "imagen" : "imágenes"}
          </Button>
        ) : (
            ...
            <Row gutter={[16, 16]} style={{ marginTop: "24px" }}>
              ...
              <Col xs={24} sm={12}>
                <Card title="Información de Contacto" type="inner">
                  {" "}
                  {!alumnoDetails.contactos ||
                  alumnoDetails.contactos.length === 0 ? (
                            ...
                            description={
                              <Space direction="vertical">
                                ...
                                {contact.contacto.celular && (
                                  <Text>
                                    Celular: {contact.contacto.celular}
                                  </Text>
                                )}
                                ...
                                {contact.contacto.direccion && (
                                  <Text>
                                    Dirección: {contact.contacto.direccion}
                                  </Text>
                                )}
                              </Space>
                            }
              ...
              <div style={{ marginBottom: "16px" }} className="no-print">
                <Space wrap>
                  <Badge
                    ...
                    text={
                      <>
                        <CheckCircleOutlined /> Pago a tiempo
                      </>
                    }
                  />
                  <Badge
                    ...
                    text={
                      <>
                        <ClockCircleOutlined /> Pago con retraso (entre 6-15
                        días)
                      </>
                    }
                  />
                  <Badge
                    ...
                    text={
                      <>
                        <WarningOutlined /> Pago con retraso mayor (más de 15
                        días)
                      </>
                    }
                  />
                ...
                <>
                  ...
                  >
                    ...
                    <Table
                      ...
                      rowKey="id"
                      pagination={
                        statements.length > 10 ? { pageSize: 10 } : false
                      }
                      onRow={(record) => {
                        ...
                        return (
                          <List.Item>
                            <Card style={cardStyle}>
                              ...
                              <div>
                                <Text type="secondary" style={textStyle}>
                                  Forma de Pago: {payment.medioPagoDescripcion}
                                </Text>
                              </div>
                              <div>
                                <Text type="secondary" style={textStyle}>
                                  Usuario: {payment.usuarioNombre}
                                </Text>
                              </div>
                              {payment.esColegiatura &&
                                ...
                                payment.anioColegiatura && (
                                  <div>
                                    <Text type="secondary" style={textStyle}>
                                      Periodo: {payment.mesColegiatura}/
                                      {payment.anioColegiatura}
                                    </Text>
                                  </div>
                                )}
                              ...
                              {payment.esAnulado && payment.fechaAnulacion && (
                                <div>
                                  ...
                                  >
                                    Fecha de anulación:{" "}
                                    {formatDate(payment.fechaAnulacion)}
                                  </Text>
                                </div>
                              )}
                              {payment.esAnulado &&
                                payment.usuarioAnulacionNombre && (
                                  <div>
                                    <Text
                                      type="secondary"
                                      style={{ color: "#ff4d4f" }}
                                ...
                                payment.imagenesPago.length > 0 && (
                                  <div style={{ marginTop: 8 }}>
                                    <Button
                                      ...
                                      icon={<EyeOutlined />}
                                      onClick={() =>
                                        handleViewImages(payment.imagenesPago)
                                      }
                                    ...
                                    >
                                      Ver {payment.imagenesPago.length}{" "}
                                      {payment.imagenesPago.length === 1
                                        ? "imagen"
                                        : "imágenes"}
                                    </Button>
                                  </div>
          ...
          {selectedImages.map((image, index) => (
            <div key={image.id} style={{ textAlign: "center" }}>
              ...
              <div style={{ marginTop: "8px" }}>
                {image.fileName && (
                  <Text type="secondary">{image.fileName}</Text>
                )}
              </div>
            </div>
          ))}
