...
const TransportPaymentsReport: React.FC = () => {
  ...
  const fetchReportData = async () => {
    if (!selectedRubroId) {
      message.warning('Por favor seleccione una ruta.');
      return;
    }
    ...
    setError(null);    try {
      const data = await makeApiRequest<PaymentReportResponse>(
        `/pagos/report?cicloEscolar=${selectedYear}&rubroId=${selectedRubroId}`, 
        'GET'
      );
  ...
  const exportToExcel = () => {
    ...
    try {
      ...
      const fileName = `ReporteTransporte_${selectedYear}_Rubro${selectedRubroId}_${new Date().toISOString().slice(0, 10)}.xlsx`;
  ...
  return (
    <div className="payment-report-container">      <div className="payment-report-header">
        <Title level={2}>Control de Pagos de Bus</Title>
      </div>
      ...
      <Card className="payment-report-filters">
        <Row gutter={[16, 16]}>
          ...
          <Col xs={24} md={12} lg={6} className="payment-report-action">
            ...
            <Col xs={24} md={12} lg={6} className="payment-report-action">
            <Space style={{ marginTop: 32 }} wrap>              <Button 
                icon={<ReloadOutlined />} 
